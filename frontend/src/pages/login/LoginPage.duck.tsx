...
export const ON_CHANGE_ROLE = "loginPage/ON_CHANGE_ROLE";
...
export const onChangeRole = (role: string) => ({
  type: ON_CHANGE_ROLE,
  payload: role,
});
...
export const handleLoginClick =
  (navigate: any) => async (dispatch: any, getState: any) => {
    ...
    if (username.trim() === "")
      return dispatch(loginFail(text["LoginPage.errorMissingFieldUsername"]));
    if (password.trim() === "")
...
export const handleSignupClick =
  (navigate: any) => async (dispatch: any, getState: any) => {
    dispatch(loginRequest());
    const { username, lastName, firstName, password, rePassword, role } =
      getState().loginPage;
    const isFullFill =
      username.trim() !== "" &&
      lastName.trim() !== "" &&
      ...
      password !== "";
    if (!isFullFill)
      return dispatch(loginFail(text["LoginPage.errorMissingField"]));
    if (password.trim() !== rePassword.trim())
      return dispatch(loginFail(text["LoginPage.errorConfirmPassword"]));
    const user: User = {
      username: username,
      last_name: lastName,
    ...
    if (res) {
      const { EC } = res;
      if (EC === 1 || EC === 2)
        return dispatch(loginFail(text["LoginPage.errorSignup"]));
      if (EC === 0) {
        const backPage = sessionStorage.getItem("backPage");
        await dispatch(getProfile());
        navigate(navigateToPage(backPage || "/"));
